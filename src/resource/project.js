 ...
 */
 function update(req, res, db) {
   ...
   req.on("end", function() {
     var project = JSON.parce(body);
     db.run("UPDATE projects SET name=?, description=?, version=?, repository=?, license=?, id=?", [project.name, project.description, project.version, project.repository, project.license, id], function(err) {
       if(err) {
         console.error(err);
         res.statusCode = 500;
